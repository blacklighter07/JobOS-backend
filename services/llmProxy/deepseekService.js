...
const { deepseekApiKey } = require("../../config/config");

const loadBalancer = new OpenAILoadBalancer(deepseekApiKey);

const getDeepSeekResponse = async ( userInput, sessionKey, modelType, botPrompt ) => {
  ...
  const updateContextAndLog = async () => {
    ...
    if (timeDiff > 60) {
      // 1 minute
      console.log("Summarizing conversation due to time limit");
  ...
  try {
    ...
    console.error("Error in DeepSeek API:", error.message);
